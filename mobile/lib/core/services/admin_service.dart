...
class AdminService {
  ...
  /// Get detailed leave report for a specific year
  Future<Map<String, dynamic>> getDetailedLeaveReport(
      Map<String, dynamic> params) async {
    try {
      print('🔄 ADMIN SERVICE: Calling detailed leave report API...');
      _addAuthToken();

      final response = await _dio.get('/admin/dashboard/leave-detail',
          queryParameters: params);
      print(
          '📊 ADMIN SERVICE: Leave report response status: ${response.statusCode}');
      print('📊 ADMIN SERVICE: Leave report response data: ${response.data}');
      ...
      return {
        'success': false,
        'message': response.data['message'] ?? 'Failed to get leave report',
      };
    } catch (e) {
      ...
      if (e is DioException && e.response != null) {
        return {
          'success': false,
          'message':
              e.response?.data['message'] ?? 'Failed to get leave report',
        };
      }
      return {
        'success': false,
